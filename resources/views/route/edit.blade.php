...
{!! Form::open(array('url' => 'route/save','files'=>true)) !!}
		<?php echo Form::text('ID',$route[0]['ID'], array('style' => 'display:none')); ?>
	<div class="form-group">
		...
		<div class="tab-content" style="padding: 0px;">
			<input class="form-control tab-pane fade in active" type="text" placeholder="ชื่อเส้นทาง(ไทย)" id="name_tha" name="name_tha" value="<?php echo $route[0]['name_tha'] ?>">
			<input class="form-control tab-pane fade" type="text" placeholder="ชื่อเส้นทาง(อังกฤษ)" id="name_eng" name="name_eng" value="<?php echo $route[0]['name_ำืเ'] ?>">
			<input class="form-control tab-pane fade" type="text" placeholder="ชื่อเส้นทาง(จีน)" id="name_cha" name="name_cha"  value="<?php echo $route[0]['name_cha'] ?>">
		</div>
	</div>
	<div class="form-group">
		...
		<div class="tab-content" style="padding: 0px;">
			<input class="form-control tab-pane fade in active" type="text" placeholder="รายละเอียดโดยย่อ(ไทย)" id="description_tha" name="description_tha" value="<?php echo $route[0]['description_tha'] ?>">
			<input class="form-control tab-pane fade" type="text" placeholder="รายละเอียดโดยย่อ(อังกฤษ)" id="description_eng" name="description_eng" value="<?php echo $route[0]['description_eng'] ?>">
			<input class="form-control tab-pane fade" type="text" placeholder="รายละเอียดโดยย่อ(จีน)" id="description_cha" name="description_cha" value="<?php echo $route[0]['description_cha'] ?>">
		</div>
	</div>
	<div class="form-group col-xs-6">
		<label class="control-label">ภาพประกอบ</label>
		<input id="pic" name="pic" type="file" class="file" data-show-upload="false" data-show-caption="true" value="<?php echo $route[0]['pic'] ?>">
		...
		<label>เริ่มต้น</label></br>
			...
			<div class="col-xs-4">
				<select class="form-control province" id="province-1" name="startProvinceID">
					...
					<?php foreach($province as $key=>$value){?>
						<option <?php if($route[0]['startProvinceID'] == $value['PROVINCE_ID']) echo 'selected'; ?> value="<?php echo $value['PROVINCE_ID'] ?>"><?php echo $value['PROVINCE_NAME'] ?></option>
					<?php } ?>
				</select>
			</div>
			...
			<div class="col-xs-4">
				<select class="form-control amphur" id="amphur-1" name="startAmphurID">
				    ...
				    <?php foreach($startamphur as $key=>$value){?>
						<option <?php if($route[0]['startAmphurID'] == $value['AMPHUR_ID']) echo 'selected'; ?> value="<?php echo $value['AMPHUR_ID'] ?>"><?php echo $value['AMPHUR_NAME'] ?></option>
					<?php } ?>
				</select>
			</div>
	</div><div style="clear:both"><br>
		...
		<label>สิ้นสุด</label></br>
			...
			<div class="col-xs-4">
				<select class="form-control province" id="province-2" name="endProvinceID">
					<option value="0">เลือกจังหวัด</option>
					<?php foreach($province as $key=>$value){?>
						<option <?php if($route[0]['endProvinceID'] == $value['PROVINCE_ID']) echo 'selected'; ?> value="<?php echo $value['PROVINCE_ID'] ?>"><?php echo $value['PROVINCE_NAME'] ?></option>
					<?php } ?>
				</select>
			</div>
			...
			<div class="col-xs-4">
				<select class="form-control amphur" id="amphur-2" name="endAmphurID">
				    <option value="0"></option>
				    <?php foreach($endamphur as $key=>$value){?>
						<option <?php if($route[0]['endAmphurID'] == $value['AMPHUR_ID']) echo 'selected'; ?> value="<?php echo $value['AMPHUR_ID'] ?>"><?php echo $value['AMPHUR_NAME'] ?></option>
					<?php } ?>
				</select>
			</div>
	</div><div style="clear:both"><br>
	...
	<div class="add-route col-xs-5 connectedSortable" style="padding: 20px;font-weight: bold;"> เส้นทาง
		<?php foreach($placelist as $key=>$value) { ?>
			<div class="select-place-list ui-sortable-handle" id="list-place-<?php echo $key ?>"><?php echo $value ?></div>
		<?php } ?>
...
<script type="text/javascript">
 ...
 //  	});
  ...
  // 	});
	...
	var routeid = "<?php echo $route[0]['ID'] ?>";
	var routeimg = "<?php echo $route[0]['pic'] ?>";
	var pathimg = "<?php echo URL::to('/'); ?>/uploads/route/"+routeid+"/thumbnail.png";
    ...
    $("#pic").fileinput({
        ...
        initialPreviewConfig: [
            {caption: routeimg, size: 930321, width: "120px", key: 1}
        ],
        // deleteUrl: "{{ url('/place/deleteimg') }}",
        autoReplace: true,
        overwriteInitial: true,
        maxFileSize: 100,
        initialCaption: routeimg,
    });
