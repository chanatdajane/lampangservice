	...
	*/
	...
	public function index()
	{
		...
		foreach($place as $key => $value){
			...
			foreach($placeCategoryList as $key2=>$value2){
				$catname = Controller::filterlang($category[$value2['categoryID']-1]['attributes']['name'],'tha');
	...
	{	
		...
		if(!empty($id)){
			...
			return view('place/edit', ['place' => $place,'category' => $category,'placeCategory' => $placeCategory,'province' => $province,'amphur' => $amphur,'placeGallery' => $placeGallery]);
		}else{
			return view('place/add',['category' => $category,'province' => $province]);
		}
	...
	public function save(){
		...
		if(!empty($data['recommend']))
			$place->recommend = $data['recommend'];
		else
			$place->recommend = 0;
